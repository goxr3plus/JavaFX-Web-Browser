...
public final class AutoCompleteTextField {
    ...
    public static final Logger LOGGER = Logger.getLogger(AutoCompleteTextField.class.getName());
     ...
     */
    private final EventHandler<? super KeyEvent> keyHandler = key -> {
        ...
        LOGGER.info(String.format("KeyCode: %s", keyCode.getName()));
        if (!(key.isControlDown() || isKeyNotAllowed(keyCode))) {
            LOGGER.info("KeyCode passed succesfully!");
    ...
    public boolean isKeyNotAllowed(KeyCode keyCode) { // Not causing problems by these buttons
    ...
    /**
     * <b> Warning with this method will lead to NullPointerException if the
     * given SortedSet goes to null , i am using it when i have really big
